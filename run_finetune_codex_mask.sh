#!/bin/bash
#SBATCH --job-name=kronos_codex_mask
#SBATCH --nodes=1
...
#SBATCH --output=/nfs/turbo/umms-drjieliu1/usr/peterszj/BioSpatialFM/finetune_codex_mask_new.log

# 使用学长已实现好的代码（含 channel mask loss），输出到自己的目录
...
# CODEX patches (primary) + islet patches (via data_path_imc)
DATA_PATH=(
    "/nfs/turbo/umms-drjieliu1/projects/HPAP-Spatial/CODEX/patches"
)
DATA_PATH_IMC="/nfs/turbo/umms-drjieliu1/usr/peterszj/BioSpatialFM/islet_patches_h5"
MARKER_METADATA="tutorials/codex_dataset/dataset/marker_info_with_metadata.csv"
MARKER_METADATA_IMC="tutorials/codex_dataset/dataset/marker_info_with_metadata.csv"
IMC_FRACTION=0.1   # CODEX:IMC ~45:1, 0.1 => ~5 IMC/batch (~4.7x oversampling)

# 输出到自己的目录（绝对路径）
OUTPUT_DIR="/nfs/turbo/umms-drjieliu1/usr/peterszj/BioSpatialFM/output/codex_mask_new"
RESUME_PATH=""
...
torchrun --nproc_per_node=2 --master-addr=127.0.0.1 --master-port=29502 finetune_kronos.py \
    --distributed \
    --bf16 \
    --data_path "${DATA_PATH[@]}" \
